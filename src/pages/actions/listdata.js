...
return(
   ...
   {datafile.slots.map((slot,index)=>
        ...
        <Box component='li' style={{listStyle:'none'}} sx={{width:400,ml:65,display:'flex'}} key={slot.start_time}>
          ...
          >{convertT(slot.start_time)}</Button>
           <Collapse  in={open===index} orientation='horizontal' timeout='auto' unmountOnExit>
           <Button sx={{ml:1,pt:2,pb:2,mt:1,width:200}} onClick ={()=>{ navigate("/slot_n")}}  variant='contained'>Confirm</Button>
          </Collapse>
        </Box>
